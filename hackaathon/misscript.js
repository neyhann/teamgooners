...
// Sample assignments data for student panel
const sampleAssignments = [
    {
        ...
        comments: 'Completed all requirements successfully'
    },
    {
        id: 2,
    ...
    },
    {
        ...
        grade: 'B+',
        files: ['calculus_ps.pdf'],
        description: 'Derivatives and applications',
        comments: 'Good work, minor errors in calculations'
    },
    {
        id: 4,
        title: 'English Essay',
        ...
        grade: null,
        files: ['essay.docx'],
        description: 'Argumentative essay on technology',
        comments: 'Submitted early for review'
    }
];
...
function initializeApp() {
    ...
    assignmentsData = [...sampleAssignments];
...
function setupEventListeners() {
    ...
    // Assignment filter
    document.getElementById('filterAssignments').addEventListener('click', filterAssignments);
    ...
    // Assignment modal close and cancel buttons
    document.querySelector('#assignmentModal .modal-close').addEventListener('click', hideAssignmentModal);
    document.querySelector('#assignmentModal .modal-cancel').addEventListener('click', hideAssignmentModal);
    // Assignment table actions
    document.getElementById('assignmentsTableBody').addEventListener('click', function(e) {
        if (e.target.closest('.btn-assignment-action')) {
            const btn = e.target.closest('.btn-assignment-action');
            const row = btn.closest('tr');
            const id = parseInt(row.dataset.assignmentId);
            if (btn.title === 'View Details') {
                viewAssignment(id);
            } else if (btn.title === 'Submit Assignment') {
                submitAssignment(id);
            } else if (btn.title === 'Download Files') {
                downloadFiles(id);
            }
        }
    });
...
function updateAssignmentsSummary() {
    const totalSubmissions = assignmentsData.length;
    const pendingSubmissions = assignmentsData.filter(a => a.status === 'pending').length;
    const submittedCount = assignmentsData.filter(a => a.status === 'submitted' || a.status === 'graded').length;
...
function renderAssignmentsTable(assignments) {
    ...
    assignments.forEach(assignment => {
        ...
        const submittedDate = assignment.submitted_date ? 
            new Date(assignment.submitted_date).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'short', 
                day: 'numeric' 
            }) : '-';
        ...
        row.innerHTML = `
            ...
            <td><span class="assignment-status ${assignment.status}">${assignment.status}</span></td>
            <td>${submittedDate}</td>
            <td>${assignment.grade || '-'}</td>
            <td class="assignment-actions">
                <button class="btn-assignment-action" title="View Details">
                    <i class="fas fa-eye"></i>
                </button>
                ${assignment.status === 'pending' ? `
                    <button class="btn-assignment-action" title="Submit Assignment">
                        <i class="fas fa-upload"></i>
                    </button>
                ` : ''}
                ${assignment.files.length > 0 ? `
                    <button class="btn-assignment-action" title="Download Files">
...
function filterAssignments() {
    const subject = document.getElementById('subjectFilterAssignment').value;
    const status = document.getElementById('statusFilterAssignment').value;
...
function showAssignmentModal() {
    ...
    document.getElementById('assignmentDueDate').value = new Date().toISOString().split('T')[0];
}
...
function handleAssignmentSubmission(e) {
    ...
    const newAssignment = {
        ...
        subject_name: document.getElementById('assignmentSubject').options[document.getElementById('assignmentSubject').selectedIndex].text,
        ...
        comments: document.getElementById('assignmentComments').value
    };
...
function downloadFiles(id) {
    ...
    if (assignment && assignment.files.length > 0) {
        alert(`Downloading files: ${assignment.files.join(', ')}\n\nNote: This is a demo. In a real application, files would be downloaded from the server.`);
    }
}
...
function changeTheme(e) {
    ...
    // Show notification with proper theme name
    let themeName = theme;
    if (theme === 'olive-green') themeName = 'Olive Green';
    else if (theme === 'light') themeName = 'Light';
    else if (theme === 'dark') themeName = 'Dark';
    else if (theme === 'space') themeName = 'Space';
    else if (theme === 'blue') themeName = 'Blue';
    else if (theme === 'purple') themeName = 'Purple';
...
function loadSavedTheme() {
    ...
    if (savedTheme) {
        ...
        // Set Olive Green as default theme
        document.documentElement.setAttribute('data-theme', 'olive-green');
        localStorage.setItem('selectedTheme', 'olive-green');
    }
}
